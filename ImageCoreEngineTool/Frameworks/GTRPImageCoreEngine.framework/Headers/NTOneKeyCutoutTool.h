...
/// 自动获取物体轮廓图(通过热力算法)
/// @param inputImage 输入图像
/// @param borderWidth 边沿宽度
/// @param density 边沿补点密度
/// @param success 处理成功的回调函数
/// @param failure 处理失败的回调函数
- (void)autoObjectContour:(UIImage *)inputImage borderWidth:(CGFloat)borderWidth density:(CGFloat)density
                  success:(void (^__nullable)(UIImage *resultImage))success failure:(void (^__nullable)(NSError* error))failure;
